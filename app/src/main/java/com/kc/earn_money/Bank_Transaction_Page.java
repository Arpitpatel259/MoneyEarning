...
public class Bank_Transaction_Page extends AppCompatActivity {
        ...
        FirebaseDatabase.getInstance().getReference().child("Wallet_Data_Entries").child(user.getUid()).child("Bank_Transaction").child("Spin_Purchase").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                    BankList.add(new PayAmountRecordModel(dataSnapshot.child("Sender_Name").getValue(String.class).toString(), dataSnapshot.child("Sender_Email").getValue(String.class).toString(), dataSnapshot.child("Transaction_Id").getValue(String.class).toString(), dataSnapshot.child("Receiver_Upi_Id").getValue(String.class).toString(), dataSnapshot.child("Spin_Purchase").getValue(String.class).toString(), dataSnapshot.child("Transaction_Status").getValue(String.class).toString(), dataSnapshot.child("amount").getValue(String.class).toString()));
                }
    ...
    private class WorkAdapter extends RecyclerView.Adapter<WorkAdapter.WorkViewHolder> {
        ...
        public void onBindViewHolder(@NonNull WorkAdapter.WorkViewHolder holder, int position) {
            ...
            String StatusConvert = holder.PayerStatus.getText().toString().toLowerCase();
            if (StatusConvert.contains("success")) {
